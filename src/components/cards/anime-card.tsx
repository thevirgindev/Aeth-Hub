...
export const AnimeCard = memo(function AnimeCard({ anime, onSelect }: { anime: { id: string; title: string; poster: string; year: number; rating: number; genres: string[]; tags?: string[]; status?: string | null; eps?: number | null }; onSelect: () => void }) {
  ...
  const onCtx = (e: React.MouseEvent) => ctx.show([
    ...
    { label: isFav ? 'Remove Favorite' : 'Favorite', icon: Heart, onClick: () => { setFavs(isFav ? favs.filter(f => f !== anime.id) : [...favs, anime.id]); showToast({ msg: isFav ? 'Removed from favorites' : 'Added to favorites', type: 'success' }) } },
  ], e)
  return (
    <div className="card-surface w-[175px] cursor-pointer group/card" onContextMenu={onCtx} onClick={onSelect}>
      <div className="relative poster-base w-full overflow-hidden rounded-b-none">
        <CachedImage src={anime.poster} alt={anime.title} title={anime.title} className="w-full h-full" />
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-deep/90 via-deep/40 to-transparent backdrop-blur-[2px] opacity-0 group-hover/card:opacity-100 transition-opacity flex flex-col justify-end p-3 z-10 rounded-b-none">
          <p className="text-sm font-bold text-text leading-tight">{anime.title}</p>
          <p className="text-[11px] text-[#FFD700] mt-1 flex items-center gap-1">
            <Star size={10} fill="#FFD700" /> {anime.rating.toFixed(1)} · {anime.year}
          </p>
          <p className="text-[11px] text-dim mt-0.5 line-clamp-2">{anime.genres.join(', ')}</p>
          <p className="text-[10px] text-muted/60 mt-0.5">{anime.status} · {anime.eps} eps</p>
        </div>
      </div>
      <div className="p-2.5">
